...
def get_threecommas_accounts_paper(logger, api):
...
def set_threecommas_ss(logger, api, thebot, raw_pairs, ss, notify=True, notify_uptodate=True):
    """Update bot with ss."""
    ...
    error, data = api.request(
        ...
        payload={
            ...
            "martingale_step_coefficient": ss,
    ...
    if data:
        ...
        if ss:
            newss = round(ss, 2)
            logger.info(
                "Bot '%s' with id '%s' updated with ss '%s'"
                % (thebot["name"], thebot["id"], newss),
                notify,
            )
        # else:
        #     logger.info(
        #         "Bot '%s' with id '%s' updated with %d SS"
        #         % (
        #             thebot["name"],
        #             thebot["id"],
        #             ss
...
def set_threecommas_bot_hmss(logger, api, thebot, newpairs, newmaxdeals, pdi, notify=True, notify_uptodate=True):
    """Update bot with new pairs."""
    ...
    if newpairs == thebot["pairs"]:
        if pdi:
            pdi_new = round(pdi, 2)
            logger.info(
                "Bot '%s' with id '%s' is already using the new pair(s) and SS of '%s"
                % (thebot["name"], thebot["id"], pdi_new),
                notify_uptodate,
            )
        return
    ...
    error, data = api.request(
        ...
        payload={
            ...
            "martingale_step_coefficient": pdi,
            "max_safety_orders": int(thebot["max_safety_orders"]),
            "max_active_deals": int(maxactivedeals),
            "active_safety_orders_count": int(thebot["active_safety_orders_count"]),
            "safety_order_step_percentage": float(
    ...
    if data:
        ...
        if len(newpairs) == 1:
            if pdi:
                pdi_new = round(pdi, 2)
                logger.info(
                    "Bot '%s' with id '%s' updated with pair '%s' and SS of '%s"
                    % (thebot["name"], thebot["id"], newpairs[0], pdi_new),
                    notify,
                )
        else:
            if pdi:
                pdi_new = round(pdi, 2)
                logger.info(
                    "Bot '%s' with id '%s' updated with %d pairs %s and SS of '%s"
                    % (
                        thebot["name"],
                        thebot["id"],
                        len(newpairs),
